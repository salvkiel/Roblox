...
for i, data in ipairs(teleportList) do
	...
	btn.MouseButton1Click:Connect(function()
		...
		checkpointValue.Value = i -- simpan checkpoint saat teleport manual
	end)
end
...
button.MouseButton1Click:Connect(function()
	...
	if teleporting then
		-- Gunakan checkpoint terakhir (manual maupun otomatis)
		local resumeFrom = checkpointValue.Value or 1
		button.Text = "STOP"
		button.BackgroundColor3 = Color3.fromRGB(0, 150, 0)
		task.spawn(function()
			-- Lanjutkan dari checkpoint terakhir
			while teleporting do
				for i = resumeFrom, #teleportList - 1 do
					if not teleporting then break end
					local data = teleportList[i]
					if safeTeleportTo(data.pos) then
						checkpointValue.Value = i
