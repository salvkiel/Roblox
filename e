...
local function autoFarmLoop()
    while teleporting do
        -- iterasi semua titik KECUALI basecamp (anggap basecamp terakhir)
        for i = 1, #teleportList - 1 do
            ...
            if data and data.pos then
                ...
                print(string.format("[AutoFarm] %s | delay %.1f detik", data.name, waitTime))
                task.wait(waitTime)
            end
        end
        ...
        -- Setelah sampai puncak, teleport ke basecamp (elemen terakhir)
        local basecamp = teleportList[#teleportList]
        if basecamp then
            safeTeleportTo(basecamp.pos)
            local waitTime = tonumber(basecamp.delay) or 3
            print(string.format("[AutoFarm] BASECAMP | delay %.1f detik", waitTime))
            task.wait(waitTime)
        end
    end
