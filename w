...
local teleportList = {
    {name = "Teleport Pos 1", pos = Vector3.new(-1.858, 324.139, 2945.717), delay = 2},
    {name = "Teleport Pos 2", pos = Vector3.new(-12.149, 336.266, 3131.099), delay = 10},
    {name = "Teleport Pos 3", pos = Vector3.new(-234.331, 401.994, 4034.662), delay = 25},
    {name = "Teleport Pos 4", pos = Vector3.new(-260.452, 339.807, 4382.458), delay = 10},
    {name = "Teleport Pos 5", pos = Vector3.new(-350.004, 331.38, 4563.691), delay = 5},
    {name = "Teleport Pos 6", pos = Vector3.new(-863.008, 228.044, 5330.454), delay = 30},
    {name = "Teleport Pos 7", pos = Vector3.new(-1381.331, 504.732, 6360.716), delay = 20},
    {name = "Teleport Pos 8", pos = Vector3.new(-1521.099, 537.663, 6747.521), delay = 20},
    {name = "Teleport Pos 9", pos = Vector3.new(-1489.38, 546.493, 7075.592), delay = 10},
    {name = "Teleport Pos 10", pos = Vector3.new(-892.735, 481.662, 8299.804), delay = 50},
    {name = "Teleport Pos 11", pos = Vector3.new(-497.311, 608.767, 8792.895), delay = 25},
    ...
    {name = "Teleport Pos 14", pos = Vector3.new(-9.714, 637.725, 9324.404), delay = 10},
    {name = "Teleport Pos 15", pos = Vector3.new(214.159, 810.763, 10017.791), delay = 25},
    {name = "PUNCAK", pos = Vector3.new(546.483, 1310.881, 10933.145), delay = 15},
    {name = "BASECAMP", pos = Vector3.new(-469.602, 61.187, 544.347), delay = 10},
}
...
-- ====== AUTO FARM SYSTEM (pakai delay tiap titik) ======
...
local function autoFarmLoop()
    while teleporting do
        -- iterasi dari 1 sampai item sebelum BASECAMP (anggap BASECAMP di akhir)
        for i = 1, #teleportList do
            if not teleporting then break end
            local data = teleportList[i]
            -- teleport ke titik
            safeTeleportTo(data.pos)
            -- tunggu sesuai delay titik itu (fallback ke 3 detik jika nil)
            local waitTime = tonumber(data.delay) or 50
            -- jika item adalah PUNCAK, lanjutkan dan setelah loop nanti balik ke basecamp
            task.wait(waitTime)
        end

        if not teleporting then break end

        -- Setelah selesai (sampai akhir daftar), teleport ke BASECAMP (anggap terakhir)
        local basecamp = teleportList[#teleportList]
        if basecamp then
            safeTeleportTo(basecamp.pos)
            -- tunggu sesuai delay basecamp atau default
            task.wait(tonumber(basecamp.delay) or 5)
        end

        -- lalu ulangi otomatis jika teleporting masih true
    end

    -- bila keluar loop, reset button tampilan di UI (aman)
...
local function toggleAutoFarm(state)
    ...
    if teleporting then
        ...
        -- set false => autoFarmLoop akan berhenti sendiri
        button.Text = "SUMMIT"
        button.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
    end
end
...
-- Optional safety: pause auto-farm saat respawn
player.CharacterAdded:Connect(function(char)
    if teleporting then
        -- pause sementara sampai character siap kembali; opsi: matikan auto farm
        toggleAutoFarm(false)
        -- beri notifikasi (bisa diganti dengan GUI)
        warn("AutoFarm dihentikan karena respawn. Klik SUMMIT untuk mulai lagi.")
    end
end)
