...
-- By Salvkiel | Integrated & Enhanced GUI by ChatGPT (2025)
...
---------------------------------------------------------------------
-- 🔹 TOMBOL SAMPING (SHOW / HIDE)
---------------------------------------------------------------------
local flagButton = Instance.new("ImageButton")
flagButton.Size = UDim2.new(0, 40, 0, 40)
flagButton.Position = UDim2.new(0, 10, 0.5, -20)
flagButton.BackgroundTransparency = 1
flagButton.Image = "rbxassetid://6031097229" -- 🏁 ikon bendera
flagButton.Parent = screenGui

-- posisi sembunyi awal
frame.Visible = false
...
local function toggleGUI()
	...
	if guiVisible then
		frame.Visible = true
		frame.Position = UDim2.new(0, -260, 0.5, -210)
		TweenService:Create(frame, tweenInfo, { Position = UDim2.new(0, 20, 0.5, -210) }):Play()
	else
		local tween = TweenService:Create(frame, tweenInfo, { Position = UDim2.new(0, -300, 0.5, -210) })
		tween:Play()
...
flagButton.MouseButton1Click:Connect(toggleGUI)
...
print("[✅ Player Detector Loaded] GUI + Toggle Button aktif (FE-safe).")
