...
-- 🔁 Fungsi untuk respawn player
local function respawnPlayer(callback)
	local character = player.Character
	if character then
		local humanoid = character:FindFirstChildOfClass("Humanoid")
		if humanoid then
			humanoid.Health = 0 -- bunuh player agar respawn otomatis
		end
	end
	player.CharacterAdded:Wait() -- tunggu sampai respawn selesai
	task.wait(2) -- beri jeda 2 detik agar stabil
	if callback then
		callback() -- panggil lagi loop setelah respawn
	end
end

local function autoFarmLoop()
	while teleporting do
		-- reset progress di awal loop
		local totalPos = #teleportList - 1 -- exclude basecamp
		local currentPos = 0
		progressBar.Size = UDim2.new(0, 0, 1, 0)
		progressLabel.Text = "0%"
		...
		-- ⚡ Naik ke setiap titik
		for i = 1, #teleportList - 2 do
			...
			if data and data.pos then
				...
				-- ✅ Update progress bar
				currentPos += 1
				local progress = math.clamp(currentPos / totalPos, 0, 1)
				local tween = TweenService:Create(progressBar, TweenInfo.new(0.5, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
					Size = UDim2.new(progress, 0, 1, 0)
				})
				tween:Play()
				progressLabel.Text = string.format("%d%%", math.floor(progress * 100))
			end
		end
		...
		if teleporting and puncak then
			print("[AutoFarm] Sampai di PUNCAK! Respawn player...")
			safeTeleportTo(puncak.pos)
			task.wait(tonumber(puncak.delay) or 3)

			-- isi penuh progress bar
			local tweenFull = TweenService:Create(progressBar, TweenInfo.new(0.5), {
				Size = UDim2.new(1, 0, 1, 0)
			})
			tweenFull:Play()
			progressLabel.Text = "100%"

			-- respawn player dan mulai ulang loop
			respawnPlayer(function()
				if teleporting then
					task.spawn(autoFarmLoop)
				end
			end)
			return -- keluar dari loop saat respawn
		end
	end

	-- Jika berhenti manual
